...
var _ = Describe("Frames", func() {
	...
	It("skips unknown frames", func() {
		...
		Expect(frame.(*dataFrame).Length).To(BeEquivalentTo(0x1234))
	})
	...
	Context("DATA frames", func() {
		It("parses", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(frame).To(BeAssignableToTypeOf(&dataFrame{}))
			Expect(frame.(*dataFrame).Length).To(BeEquivalentTo(0x1337))
		})
		...
		It("writes", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(frame).To(BeAssignableToTypeOf(&dataFrame{}))
			Expect(frame.(*dataFrame).Length).To(BeEquivalentTo(0xdeadbeef))
		})
	})
	...
	Context("HEADERS frames", func() {
		It("parses", func() {
			...
			Expect(frame.(*headersFrame).Length).To(BeEquivalentTo(0x1337))
		})
		...
		It("writes", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(frame).To(BeAssignableToTypeOf(&headersFrame{}))
			Expect(frame.(*headersFrame).Length).To(BeEquivalentTo(0xdeadbeef))
		})
	})
